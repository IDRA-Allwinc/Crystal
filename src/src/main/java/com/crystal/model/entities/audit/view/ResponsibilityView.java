...
@Entity
@Subselect("select c.id_responsibility, c.number, concat(substring(c.description, 1,30),'...') description, date_format(c.init_date, '%d/%m/%Y') initDate, date_format(c.end_date, '%d/%m/%Y') endDate, c.is_attended isAttended, a.id_audit auditId,\n" +
        "case " +
